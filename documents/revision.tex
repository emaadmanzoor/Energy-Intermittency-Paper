...
%	\date{\vspace{-1em}}
	...
	% In this paper, we develop a model where consumers demand electricity over time 
	...
	In this paper, we develop a model where we capture the effects of intermittency on the adoption of energy technologies. We do so through the demand-side of the market; specifically, we model consumers using a utility function that implies a preference for smoothing electricity consumption over time. We then parameterize our model empirically, implement our model numerically, and study the results. 
	...
	\end{equation}
	Combining this FOC with the demand equation (\cref{eq:demand}) allows us to find the equilibrium. Generally, the equilibrium results for any number of technologies ($n$) are analytic, but they are difficult to interpret algebraically due to the number of parameters involved. 
	...
	\begin{proposition}
		Assume that, for all technologies $i$ and  periods $t$, we have $\xi_{i,t} > 0 \, , \alpha_t > 0 \, ,$ and $ c_i > 0$. For technology $j$ to be economical, it must meet three conditions with respect to some arbitrary period $s$. Firstly, it must be more cost efficient than any of the other technologies in period $s$. Secondly, it must have  a relative advantage in output efficiency in the same period $s$. And, thirdly, there must be sufficient amount of demand for energy in this period $s$. 
	\end{proposition}
	The first condition is on cost efficiency and is fairly intuitive. Consider its contrapositive: if a technology does not have an advantage in cost efficiency in any period, it will not be used; this is fairly obvious. Alternatively, if a technology is the most cost effective in every single period, it will be the only technology used. The second condition regarding output efficiency actually stems from the invertibility of $\boldsymbol{\xi}$. If we did not have $\boldsymbol{\xi}$ invertible, we either have at least one technology that does not produce in any period or we have at least one technology being a linear combination of the other technologies in terms of output. In the latter case, some technologies are not economical because their output can be replicated by the other technologies in a more cost effective way. That is, suppose $\boldsymbol{\xi}$ consisted of 3 technologies but was of rank $2$. In this case, we may represent any of these technologies as a linear combination of the other two; call this combination the synthetic version of technology $i$. It is not possible for every synthetic technology $i$ to be more expensive than its original; for at least one $i$, we must have a synthetic technology being cheaper or equal in price to the actual technology. In the first case, synthetic technology $i$ is cheaper so we do not use the actual technology $i$. In the latter case, we may still eliminate technology $i$ or not use the other two technologies depending on the demand. Overall, in any case, $\boldsymbol{\xi}$ not invertible means that at least one technology is not used. Finally, the demand condition is straightforward; even if a technology is optimal in a certain period, if consumers do not sufficiently demand electricity during that period, then there is little reason to use that technology. 
	
	We may also derive the comparative statics for this simplified scenario.  
	\begin{proposition}
		Suppose that the conditions of Proposition 1 hold for each technology, so we are not in an edge case. The equilibrium quantity of a technology is increasing with its output efficiency and decreasing with its cost; at the same time, it is decreasing with the output efficiency of the other technologies and increasing with their cost. Also, suppose that some technology $i$ is the most cost effective in period $t$. Then, its equilibrium quantity is increasing with respect to the demand parameter $\alpha_t$ and decreasing with respect to the demand parameters of the other periods. Furthermore, again assuming technology $i$ is the most cost effective in period $t$, the comparative statics of $Z_t$ and $X_i$ are equivalent. 
	\end{proposition}	
	The comparative statics with respect to $X_i$ and output efficiency and cost of technology $i$ are not surprising. On the other hand, the statics for $Z_t$ may not be immediately obvious. Instead, they follow from the fact that we have $\mathbf{Z} \equiv \boldsymbol{\xi}^T \mathbf{X}$. That is, suppose we have an arbitrary technology $i$ that is the most cost effective source of electricity in period $t$. If consumers demanded that 100\% of their energy arrive in period $t$, then relying solely on technology $i$ for energy would be the most economical solution. Consequently, it seems intuitive that the comparative statics of $X_i$ follow through to $Z_t$. This intuition just happens to apply even when other technologies are employed and there is demand in multiple periods. Similarly, the comparative statics for the share parameters of the utility function, $\boldsymbol{\alpha} \equiv \left( \alpha_t \;\; \alpha_s \right)^T$, travel in the opposite direction. A rise in $\alpha_t$ would directly raise the optimal quantity of $Z_t$; hence, whichever technology is most cost effective at producing in period $t$ would be used more. We provide a more detailed and formal discussion of the comparative statics and edge cases in the appendix. 
	
	Furthermore, these theoretical results have assumed that $\sigma = 1$. This simplification was made for analytical tractability; specifically, it implies that the utility function is Cobb-Douglas which then results in the first order condition $Z_t = \alpha_t/p_t$. This condition is vital to facilitating the analysis of $\mathbf{X}$, since, by definition, we have $\mathbf{Z} \equiv \boldsymbol{\xi}^T \mathbf{X}$.	However, this simplifying assumption on $\sigma$ may not be realistic. Hence, in the following section, we empirically estimate $\sigma$ to parameterize the model. Since the empirical estimate is not exactly 1, this complicates the solution for $\mathbf{Z}$; consequently, the solution for $\mathbf{X}$ loses tractability. However, we can still study the model in a qualitative sense to see how values of $\sigma$ would affect the equilibrium. 
	
	\begin{proposition}
		Suppose that we have $\sigma \to \infty$. Then, electricity consumption in each period $t$ is a perfect substitute for electricity consumption in period $s$ for all periods $t,s$. Secondly, the utility function takes on the linear form $U = \sum_t \alpha_t Z_t$. And, thirdly, the set of optimal bundles of inputs $\mathbf{X}$ is given by 
		$$
		W =
		...
		where $S$ represents the set of indices for technologies that have maximimal cost-adjusted marginal utility. In other words, any vector of inputs $\mathbf{X}$ consisting of a combination of technologies in $S$ represent a valid equilibrium solution. Furthermore, the set $Y = \{ \mathbf{Z} \,: \, \mathbf{Z} = \boldsymbol{\xi}^T \mathbf{X}  \; \, \forall \, \mathbf{X} \in W \}$ contains all possible equilibrium values of electricity output. 
	\end{proposition}
	
	If we have $\sigma \to \infty$, then electricity in each period becomes a perfect substitute for electricity in other periods. This then leads to multiple possible solutions. The mathematical reason is that , since the utility function becomes linear, the marginal utility of each input becomes $\sum_t \alpha_t \xi_{i,t}$; then, using only whatever input offers the highest cost-adjusted marginal utility $(1/c_i) \sum_t \alpha_t \xi_{i,t}$ will maximize utility. If there are multiple technologies that have the same cost-adjusted marginal utility, we may get multiple solutions. The intuition here is that the perfect substitutability of $Z_t$ implies that consumers do not care about smoothing their electricity consumption over time. But, they still value consumption in particular periods more than others through the $\boldsymbol{\alpha}$ parameter. Consequently, they simply use whatever technology gives them the most total electricity (weighted by $\boldsymbol{\alpha}$). Intermittency of output simply does not matter in this case. This is what we should expect given that $\sigma \to \infty$ implies perfect substitutability. A detailed proof is given in \hyperref[sec:cobbdoug]{Appendix A.A}. 
	
	On the other hand, suppose that $\sigma \to 0$. This is the standard case of perfect complements in which we would get $U = \min_t \, Z_t/\alpha_t$. Unlike the perfect substitute case, any analytic results would not be as clear. However, some of the intuition from before carries through. That is, when electricity consumption in each period is perfectly complementary with that in other periods, consumers care strongly about smoothing their consumption. For instance, consider again a two period, two technology case with solar (intermittent) and coal (constant-output); furthermore, suppose that $\alpha_t$ is constant. The utility function here implies that consumers only care about periods when they are getting the least electricity. Since solar performs worse during the off-peak, the implicit demand for each technology would solely be based on their off-peak performance. Interestingly, no matter how high solar's output efficiency is during peak hours, as measured by its output per unit, consumer's decisions would be unaffected. In other words, intermittency plays a much larger role in this case, since \textit{when} a technology produces 
	
	To summarize, the channel by which $\sigma$ affects the inputs $\mathbf{X}$ is through the demand for electricity. Specifically, $\sigma$ is inversely related to how much consumers value smoother electricity consumption $\mathbf{Z}$. When $\sigma$ is low, consumers care more about consumption smoothing. They are willing to reduce their total consumption of electricity if it means having more consistent electricity over time. In this case, intermittent energy technologies are less appealing because, by definition, they do not generate a consistent electricity output over time. On the other hand, suppose $\sigma$ is high. Then, consumers care less about consumption smoothing and are focused more on total consumption; in the extreme case above, when $\sigma \to \infty$, they only care about total consumption and not about when they get their electricity. In this case, the intermittency is relatively less of a concern; consequently, other factors like cost efficiency and output efficiency play a larger role. In short,  $\sigma$ is inversely related to how much the intermittency of an input matters. 
	...
	}  from here on, we will refer to this latter elasticity as $e$. This elasticity is important, because sufficient substitutability between these energy inputs  is required to transition into greener economy in the future. For instance, \citet{Ace2012} provide a model where they argue that, ``When the two	sectors [clean and dirty energy] are substitutable but not sufficiently so, preventing an environmental disaster requires a permanent policy intervention. Finally, when the two sectors are complementary, the only way to stave off a disaster is to stop long-run growth."

	Now, to clarify our discussion, we first look at a general model of energy production common in the literature. Specifically, we have
	\begin{equation}
	\tilde{Z} = \left( \sum_i \beta_i \tilde{X}_i^\omega \right)^{1/\omega}
	\end{equation} 
	where   $\tilde{Z}$ is the electricity produced, $\tilde{X}_i$ is the quantity of electricity-producing input, $\beta_i$ is a share parameter, and $(\omega-1)^{-1} $ is the elasticity of substitution between the inputs. In other words, the literature often represents electricity production using a CES function and treats electricity as a single, homogeneous good. Furthermore, such models would generally have firms maximizing profit against a  demand function based on some utility-maximizing agent. Although this approach is not realistic in its assumptions, it captures the imperfect substitution between energy inputs in a tractable way. However, any results based on this model would be based on the assumption that the  elasticity of substitution between energy inputs, $e$, is constant. 
	
	On the other hand, our model is based on relatively more realistic assumptions but it implies that $e$ is non-constant. We arrive at this conclusion through the numerical results which will follow, although we can also see that $e$ is non-constant in a simpler setting. Specifically, when $\sigma = 1$ and we have two periods and two technologies, the elasticity of substitution is given by\footnote{The full derivation is given in  \hyperref[sec:EOS_derivation]{Appendix A.A}.}
	...
	\end{equation}
	It is clear from the above equation that $e$ depends on $\mathbf{c}$. However, what is not clear is the extent to which $e$ varies with $\mathbf{c}$ whether it varies enough to make a CES production assumption problematic. Furthermore, the above mathematical expression is only valid for $\sigma = 1$ which is close to but not exactly our estimates for $\hat{\sigma}$. Whether or not $e$ becomes constant under different values of $\sigma$ is another question. Because $e$ does not have a tractable solution when $\sigma \neq 1$, we parameterize and implement our model numerically in order to better study this elasticity.  
...
%	To start, we consider how $\sigma$ affects the substitutability of fossil and renewable energy. This is important, because sufficient substitutability between these two technologies is required to transition into greener economy in the future. For instance, \citet{Ace2012} provide a model where they argue that, ``When the two	sectors [clean and dirty energy] are substitutable but not sufficiently so, preventing an environmental disaster requires a permanent policy intervention. Finally, when the two sectors are complementary, the only way to stave off a disaster is to stop long-run growth." However, note that while our model sets the intertemporal elasticity of substitution for  electricity consumption $\sigma$ to a fixed value, we make no direct assumptions about the elasticity of substitution between different energy technologies. Instead, this latter elasticity is indirectly defined by the substitutability of electricity consumption between periods. To illustrate this, we parametrize and estimate our model numerically.
	...
	We plot our numerical estimates of $e_{1, 2}$, the elasticity of substitution between solar and coal power, in \autoref{fig:eosnum}. We generated these results by computing the optimal quantities of each technology given a range of relative prices $c_2/c_1$. Then, we numerically differentiated $\log(X_1/X_2)$ with respect to $\log(c_2/c_1)$ to estimate the elasticity of substitution between solar and coal  $e$. This is done by computing the slope of the first subplot in \autoref{fig:eosnum} at each value for $\log(c_2/c_1)$. Since $\log(X_1/X_2)$ is rising at an increasing rate with respect to $\log(c_2/c_1)$, we see that $e$ is positive and increasing in the lower subplot.  We repeat this process with different values of $\sigma$, the IES in the consumer's utility function. Specifically, we use our estimate of $\hat{\sigma} = 0.8847\, (0.044)$ and its 95\%  confidence interval $(0.7985, 0.9709)$.  Finally, after finding the optimal values of our quantities $\mathbf{X}$, we only keep results in our numerical simulation that correspond to interior solutions; that is, we drop edge cases where optimal output or technology  is non-positive. 
	
	
	
	The first relationship we see in \autoref{fig:eosnum} is that $e_{1,2}$ varies non-linearly with the relative costs of each technology; in particular, it appears to take on a hockey stick shape.  It is quite similar to the mathematical expression for $e$ when $\sigma = 1$ given in the earlier (\cref{eq:eosmath}). However, it is more easily understood intuitively. Specifically, this shape can be understood as the interaction of two effects: intermittency and costs. The \textit{intermittency effect} is that $e$ falls when our electricity output is highly intermittent. The \textit{cost effect} is that $e$ is high costs $(c_1, c_2)$ differ significantly while $e$ is low  when costs are closer together.
	
	First, we qualify the intermittency effect.  Suppose that a majority of our energy came from solar; this corresponds to the left side of the graph where $\log(c_2/c_1)$ is lower and, as shown in the  first subplot, $\log(X_1/X_2)$ is higher. In this case, the overall output of electricity is intermittent. Since most of the supply comes from solar, we have a majority of electricity arriving during peak hours and much less during the off-peak. The intermittency of the overall supply is high here, since consumers are being starved of electricity during the off-peak. So, it is difficult to replace even more of the coal input with solar. Therefore, $e$ is relatively low. On the other hand, suppose that a majority of our electricity supply comes from coal; this corresponds to the right side of \autoref{fig:eosnum}. In this case, the electricity output is quite consistent over time. Consequently, replacing a marginal amount of coal input with solar input is not an issue. That is, making the overall energy output slightly more intermittent when it is already stable does not create as much disutility.  Hence, the elasticity $e$ is high, since it is easy to substitute between the energy inputs.  In short, as the electricity output becomes less intermittent, $e$ should rise. 
	
	Next, we study the effects of costs on $e$. We see quantities become more sensitive to costs when costs greatly differ.\footnote{We explore this point further in Appendix B in which we consider a numerical example where technologies are minimally intermittent. That is, we again consider a two-period, two-technology example, but assume that $\xi_1$ and $\xi_2$ are nearly constant over time. We plot our results in \autoref{fig:eosrange} and find that the elasticity of substitution takes on a u-shape. In other words, when intermittency is not a concern as a result of the output parameters $\boldsymbol{\xi}$, we see the cost effect take over. 
	} To understand why, suppose that cost efficiencies of two inputs are drastically different. Then, independent their intermittency, it is likely that the equilibrium quantities of the inputs are primarily being driven by costs. For example, note that consumers want more electricity during peak hours compared to the off-peak. They can only achieve this by combining solar and coal, since the electricity output of coal is constant while solar can produce more during peak hours. Then, if solar is extremely expensive, this makes it expensive to get relatively more electricity during peak hours than off-peak hours.  Consequently, it may actually be more economical for consumers rely primarily on coal. In other words, they would have to give up too much of their income to get more power during peak-hours than off-peak hours.  Now, note that the elasticity $e$ refers to how log inputs change with respect to their costs. Consequently, when costs are very different, we would expect that small changes in costs would do much to change quantities, since costs (as opposed to other factors like intermittency) are primarily driving the market equilibrium. So, $e$ would be high when $\log(c_2/c_1)$ takes extreme values. On the other hand, when costs are similar, the cost of generation does not matter for consumers as much as intermittency and other effects; so, we would expect $e$ to be low when $\log(c_2/c_1)$ takes more intermediate values. 
	
	We can add up the intermittency and cost effects to understand the shape of $e$. This means looking at three cases. The first case is the left-hand side of the second subplot of \autoref{fig:eosnum} where solar is much cheaper. Here, the overall energy supply is highly intermittent so $e$ gets strongly pushed downwards; at the same time the cost effect pushes $e$ upwards. These two effects cancel out and we see $e$ take an intermediate value. The second case is near the middle of the subplot where we have more intermediate costs. Here, the intermittency effect moderately increases $e$ while the cost effect moderately decreases $e$. So, again, $e$ takes an intermediate value. Finally, the third case is the right-hand side of the subplot where the majority of the electricity comes from coal. The intermittency effect here pushes $e$ upwards, and, similarly, the cost effect raises $e$. So, in this case, we see $e$ being relatively high.
	...
	Additionally, note that the elasticity of substitution $e_{1,2}$ between solar and coal becomes larger and more non-linear as $\sigma$ rises. Firstly, the reason why  it becomes larger at all points is fairly intuitive. As discussed earlier, when $\sigma$ is higher, consumers care less about smoother consumption and intermittency is less of an issue. Consequently, consumers are more willing to accept intermittency in the electricity supply and thus the elasticity $e$ rises. Next, to understand the non-linear effect and change in shape, we can appeal to the intermittency effect and the cost effect. Note that $\sigma$ rising makes intermittency matter less; this diminishes the intermittency effect. As a result, we see the cost effect dominate. Recall that the cost effect creates a u-shape while the intermittency effect is what causes $e$ to fall when we rely heavily on solar. Now, note how when $\sigma$ rises, the shape of $e$ becomes more u-shaped. This is because the intermittency effect becomes more diminished and the cost effect takes over.  By this logic, if we keep raising $\sigma$, the level of $e$ becomes increasingly larger; at the same time, we would also see it become more u-shaped as the cost effect takes over.
	
	We can tie these results in with the more mathematical discussion of the model's equilibrium from earlier. Specifically, recall that when $\sigma \to \infty$, utility becomes linear and the equilibrium tends towards whatever inputs maximize total electricity output. On the other hand, when $\sigma \to 0$, utility becomes Leontiff and consumers want to completely smooth their electricity consumption. These two extreme cases can help us understand how $e$ changes with $\sigma$. 
	
	In the first case, when $\sigma \to \infty$, recall that the optimal solution was to only use whatever input had the highest cost-adjusted marginal utility $\sum_t \alpha_t \xi_{i,t} /c_i$. The intuition here is that a very high $\sigma$ causes electricity in each period to become a perfect substitute for electricity in other periods; this can be seen by the fact that utility becomes a linear function of electricity output $\mathbf{Z}$ in each period. So, consumers only care about their total electricity consumption rather than when they get their electricity.  As a result, all solutions turn into edge cases. Consequently, small changes in the cost of input $i$ can cause its equilibrium value to go from its maximum level to zero. Since $e_{12} = \partial \log(X_1/X_2) / \partial \log(c_2/c_1)$, we expect $e$ to be extremely large in this case since quantities are very sensitive to prices. 
	
	In the second case, when $\sigma \to 0$, the optimal solution was to figure out some combination of resources that make consumption as smooth as possible. Specifically, the utility was given by $\min_t \, Z_t/\alpha_t$. Therefore, consumers want a relatively fixed ratio of peak and off-peak consumption.  Here, we know that coal offers a strictly smooth output while solar offers more during peak hours. Hence, the equilibrium would be one which primarily uses coal and then a small amount of solar to account for the fact that consumers demand slightly more electricity during peak hours. Furthermore, even if costs change, the optimal solution would still require the consumer to maintain a relatively fixed ratio of peak and off-peak electricity cosnumption to maximize utility; this then requires maintaining a fixed ratio of solar and coal. Consequently, changes in the cost of solar and coal would do little to affect the equilibrium ratio of inputs. Thus, since $e$ measures how sensitive the two inputs are with respect to their costs, it would be extremely small in this case. 
	 
	To summarize, $e$ depends on two factors, cost and intermittency, while $\sigma$ affects the importance of intermittency. When costs are highly different, $e$ rises -- this is the cost effect. When the output of electricity is highly intermittent, $e$ falls -- this is the intermittency effect. These two effects combine to give $e$ the shape we see in \autoref{fig:eosnum}. Furthermore, at lower levels of $\sigma$, consumers case more about smoother electricity consumption and thus intermittency matters more. So, lower $\sigma$ means the intermittency effect is larger and $e$ is smaller overall. Additionally, when the intermittency effect is large, we see $e$ become more linear, while, when the intermittency effect is small, $e$ becomes more u-shaped as the cost effect dominates. Overall, these numerical results for $e$ are consistent with the analytic results and intuition discussed earlier. 
