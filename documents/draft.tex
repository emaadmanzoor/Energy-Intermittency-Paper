...
\begin{table}[!t]
	...
	\begin{tabular}{@{\extracolsep{0em}}c}
		...
		\multicolumn{1}{@{\hspace{0.2in}}p{5.9in}@{}}{ \textit{Note: } These results were obtained using the following parameters: $\alpha_t = 0.6$, $\alpha_s = 0.4$, $\xi_1 = (1, \, 1)$, $\xi_2 = (1, \, 0.1)$, $c_1 = 104.3$, $c_2 = 60$, $\sigma = 0.5$. For the first subplot, we plot the optimal quantity of coal while varying $\xi_2$, the output efficiency of solar, by different factors; so, for instance, at 100\% on the x-axis, we have $\xi_2 = (2, \, 0.2))$. Similarly, in the second subplot, we do the same but with the cost of coal, $c_1$.    }     \\  
	\end{tabular}
\end{table}
