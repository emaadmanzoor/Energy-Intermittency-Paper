...
nl (ln_all_sec_gen = {c=1} + (exp({ln_alpha_solar=0})*(ln_solar_gen_hat^{phi}) + exp({ln_alpha_wind=0})*(ln_wind_gen_hat^{phi}) + exp({ln_alpha_geothermal=0})*(ln_geothermal_gen_hat^{phi}) + exp({ln_alpha_biomass=0})*(ln_biomass_gen_hat^{phi}) + exp({ln_alpha_nuclear=0})*(ln_nuclear_gen_hat^{phi}) + exp({ln_alpha_hydro=0})*(ln_hydro_gen_hat^{phi}) + exp({ln_alpha_fossil=0})*(ln_fossil_gen_hat^{phi}))^(1/{phi=1})), hasconstant(c) iterate(200) nolog
